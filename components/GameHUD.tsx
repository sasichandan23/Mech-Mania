    ...
    <div className="w-full bg-slate-900/90 border border-mech-border rounded-xl p-3 sm:p-4 backdrop-blur-md shadow-2xl space-y-3">
      ...
      <div className="flex items-center justify-between border-b border-mech-border/60 pb-2.5">
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="text-xs sm:text-sm font-bold text-slate-200 tracking-wide flex items-center gap-1.5">
              <span className="truncate max-w-[110px] sm:max-w-[200px]">{playerName}</span>
              <span className="text-[9px] sm:text-[10px] font-mono text-slate-500 shrink-0">[{participantId}]</span>
            </div>
            <div className="flex items-center gap-2 mt-0.5">
              <span
                className={`text-[9px] sm:text-[10px] font-mono uppercase px-1.5 sm:px-2 py-0.5 rounded font-bold ${
                  isBossLevel
      ...
      {(activeDoubleXP || activeShield) && (
        <div className="flex flex-wrap items-center gap-1.5 pt-1">
          {activeDoubleXP && (
            <div className="px-2 py-0.5 bg-yellow-500/20 border border-yellow-500/40 rounded text-[10px] sm:text-xs font-mono text-yellow-300 flex items-center gap-1 animate-pulse">
              <Zap className="w-3 h-3 text-yellow-400" />
              2X XP ACTIVE
            </div>
          )}
          {activeShield && (
            <div className="px-2 py-0.5 bg-cyan-500/20 border border-cyan-500/40 rounded text-[10px] sm:text-xs font-mono text-cyan-300 flex items-center gap-1">
